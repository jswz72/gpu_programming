...
__global__ void shortest_path_weights_kernel(long *beg_pos, long *adj_list, int *weight, int vert_count, int source, int *distances)
{
    ...
    __shared__ int min_idxs[128];
    __shared__ int min_dists[128];
    __shared__ int cur = source;
    ...
    for (int count = 0; count < vert_count - 1; count++)
    {
        ...
        // Update distances
        min_dists[threadIdx.x] = INT_MAX;
        int tidx = tid;
        //for (int i = beg_pos[cur]; i < beg_pos[cur+1]; i++)
        while (beg_pos[cur] + tidx < beg_pos[cur + 1])
        {
            int i = beg_pos[cur] + tidx;
			int neighbor = adj_list[i];
            if (!path[neighbor] && 
                    distances[cur] != INT_MAX &&
                     distances[cur] + weight[i] < distances[neighbor])
            {
                int dist = distances[cur] + weight[i];
                distances[neighbor] = dist;
                if (dist < min_dists[threadIdx.x]) {
                    min_dists[threadIdx.x] = dist;
                    min_idxs[threadIdx.x] = neighbor;
                }
            }
        ...
        for (int i = blockDim.x / 2; i > 0; i /= 2) {
            if (threadIdx.x < i) {
                if (min_dist[threadIdx.x + i] < min_dist[threadIdx.x]) {
                    min_dist[threadIdx.x] = min_dist[threadIdx.x + i];
...
int main(int argc, char **argv) {
    ...
    HANDLE_ERR(cudaMemset(dist_mat_d, INT_MAX, csr->vert_count * num_source_words));
